> **Рабочий вход обновлён 5 сентября 2026 года.** Все части проекта находятся в
> `/Users/afonin900/Github/Margariteros/`; карта — [сохранённая карта общей папки](docs/local-workspace/README.md).
> Новая работа: `margariteros-site` — сайт; `margariteros-content` — материалы
> и Buffer; `margariteros-analytics` — измерение; `margariteros-ads` — реклама;
> `margariteros-r-club` — партнёры; `margariteros-booking-service` — бронирование.
> Этот репозиторий сохраняет историю и общие связи задач. Сведения ниже о
> расположении частей и состоянии сервисов могут быть историческими;
> проверяй текущие факты в выбранном репозитории и его GitHub-задачах.
> Ограничения на секреты, удаление и внешние изменения продолжают действовать.
> Перед работой прочитай `AGENTS.md` выбранной части и документ её направления.
...
- Контракты: `site/src/lib/consent/` — versioned first-party cookie и `readConsent/saveConsent/subscribeConsent`; `choice-consent-bridge` синхронизирует подтверждённую на live 2026-08-31 общую куку ChoiceQR `cookieSettings` на `.margariteros.bar`, но формат не считается публично стабильным договором поставщика. `site/src/lib/analytics/attribution.ts` переносит только allow-listed UTM/click IDs без PII. `site/src/lib/analytics/` — единственный consent-gated/deduped вход в `dataLayer`/GTM; PII запрещены также в логах и GitHub.
