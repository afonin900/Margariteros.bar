...
| R07 | «использовать его в виде чат-бота, чат Телеграма, сайта» | done | Общий RefRef `/club` поднят как web/Mini App surface; реальный bot registration остаётся отдельным шагом | ticket 05, 06 |
...
| R24 | «заложить основу этой партнерской системы» | done | Официальный RefRef, native Syrve adapter boundary, PostgreSQL и R Club portal работают в штатном Dokploy | ticket 03, 04, 05, 06 |
...
| R27 | «желательно пилот, чтобы был запущен» | done | После прямого разрешения владельца R Club развернут отдельным compose в штатном Dokploy; HTTPS и `/api/ready` отвечают 200 | ticket 06 |
...
| D03 | *(обнаружено в ticket 06)* локальный Docker Desktop запущен, но engine/socket не отвечает | resolved | Владелец выбрал штатный Dokploy; remote runtime принят по HTTPS и health/readiness, локальный engine больше не блокирует пилот | ticket 06 |
