...
import { checkSyrveRuntimeReadiness, readSyrveRuntimeConfig } from "../services/syrve-native.js";
...
async function healthHandler(request: FastifyRequest, reply: FastifyReply) {
  ...
  const syrveConfigured = Boolean(readSyrveRuntimeConfig());
  const syrveReady = syrveConfigured && await checkSyrveRuntimeReadiness();
  checks.syrveAdapter = syrveReady
    ? { ok: true, status: "ready" }
    : {
        ok: false,
        status: "not_ready",
        error: syrveConfigured
          ? "syrve_readback_unavailable"
          : "missing_syrve_configuration",
      };
