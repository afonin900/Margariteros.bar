...
export async function verifyResponsiveUi() {
  ...
  try {
    ...
    for (const [width, height] of viewportCases) {
      ...
      const interaction = await cdp.send("Runtime.evaluate", { expression: `(() => { const drawer=document.querySelector('[data-drawer]'); const menu=document.querySelector('[data-drawer-toggle]'); const desktop=innerWidth>760; const language=document.querySelector(desktop?'[data-language-desktop]':'[data-language-mobile]'); const languageButton=document.querySelector(desktop?'[data-language-open="desktop"]':'[data-language-open="mobile"]'); menu?.click(); const drawerOpen=!drawer?.hasAttribute('hidden') && document.documentElement.classList.contains('cq-scroll-lock'); document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape'})); const drawerEscaped=drawer?.hasAttribute('hidden') && !document.documentElement.classList.contains('cq-scroll-lock'); languageButton?.click(); const languageOpen=!language?.hasAttribute('hidden'); document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape'})); return { drawerOpen, drawerClosed: drawer?.hasAttribute('hidden'), drawerEscaped, languageOpen, languageClosed: language?.hasAttribute('hidden') }; })()`, returnByValue: true });
